...
steam.prototype.validate = function(obj, method) {
  ...
  switch(method) {
    case 'getNewsForApp':
      if ( typeof obj.appid != 'string' && typeof obj.appid != 'number') obj.callback('invalid appid');
      if ( typeof obj.count != 'string' && typeof obj.count != 'number') obj.callback('invalid count');
      if ( typeof obj.maxlength != 'string' && typeof obj.maxlength != 'number') obj.callback('invalid maxlength');
      break;
    case 'getGlobalAchievementPercentagesForApp':
      if ( typeof obj.gameid != 'string' && typeof obj.gameid != 'number') obj.callback('invalid gameid');
      break;
    case 'getPlayerSummaries':
      if (!obj.steamids) obj.callback('invalid steamids');
      if (typeof obj.steamids == 'object' && !obj.steamids.length) {
        obj.callback('getPlayerSummaries steamids only accepts a string or array of strings');
      }
      if (typeof obj.steamids == 'object' && obj.steamids.length > 100) obj.callback('too many steamids');
      break;
    case 'getSchema':
      if (!obj.gameid || (typeof obj.gameid != 'string' && typeof obj.gameid != 'number')) {
        obj.callback('invalid gameid');
      }
      break;
    case 'getPlayerItems':
      if (!obj.gameid || (typeof obj.gameid != 'string' && typeof obj.gameid != 'number')) {
      ...
      if (typeof obj.steamid != 'string') {
        obj.callback('getPlayerItems steamid argument only accepts a string');
      }
      break;
  }
}
